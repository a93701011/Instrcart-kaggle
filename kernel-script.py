...
#usesrweekone = pd.get_dummies(userweek['order_dow'])
#usesrweekone.columns =['W1','W2','W3','W4','W5','W6','W7']
...
for row in priors.itertuples():
    ...
    if z not in d:
        d[z] = (1,0)
              ...
              # row.add_to_cart_order, row.reordered)
    else:
        d[z] = ([z][0] + 1,1)
                #max(d[z][1], (row.order_number, row.order_id)),
                #d[z][2] + row.add_to_cart_order, row.reordered)

print('to dataframe (less memory)')
userXproduct = pd.DataFrame.from_dict(d, orient='index')
del d
userXproduct.columns = ['nb_orders','reordered']
userXproduct.nb_orders = userXproduct.nb_orders.astype(np.int16)
#userXproduct.columns = ['nb_orders', 'last_order_id', 'sum_pos_in_cart','reordered']
#userXproduct.nb_orders = userXproduct.nb_orders.astype(np.int16)
...
### train / test orders ###
train_orders = orders[orders.eval_set == 'train'].loc[:,['order_id','user_id']]
test_orders = orders[orders.eval_set == 'test'].loc[:,['order_id','user_id']]
...
def features(selected_orders, labels_given=False):
    ...
    df['UP_reordered'] = df.z.map(userXproduct.reordered)
...
f_to_use = ['user_total_orders', 'user_total_items', 'total_distinct_items',
       ...
       'order_hour_of_day', 'days_since_prior_order', 'days_since_ratio','user_dow',
       'aisle_id', 'department_id', 'product_orders', 'product_reorders',
       'product_reorder_rate', 'UP_orders', 'UP_orders_ratio',
        'UP_reorder_rate', 'UP_reordered'] # 'dow', 'UP_same_dow_as_last_order'
